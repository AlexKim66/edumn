<template>
    <div class="dg">
        <div class="dg-body"
            ...
            <div :class="`dg-table ${header != null ? 'custom-table' : ''} ${'template-'+template}`">
                    ...
                    <tr v-for="tr in header.structure" :key="tr.index">
                        <td
                            ...
                            <div :style="`width: ${td.width}; height: ${td.height}`">
                                <div :style="td.rotate ? 'transform: rotate(-90deg);' : ''">{{ td.label }}
                                </div>
                            </div>
                        </td>
                    </tr>
                ...
                <ag-grid-vue
                    ...
                    ${header != null ? 'custom-table-template' + template  : 'template-'+template}
            ...
            <div class="dg-footer">
                <div class="dg-info">
                    {{`${$static_words ? $static_words.total : 'Нийт'}`}}: {{ info.total }}
                ...
                <Page v-if="!this.isClient" size="small" :current="query.currentPage"
                      ...
                      :total="info.total" show-elevator @on-change="changePage"></Page>
            </div>
        </div>
        ...
        <paper-modal
            ...
            width="78%"
            height="90%">
            <excel-import v-if="isExcelUpload"
                          :schemaID="$props.schemaID"
                          :header="header"
                          :schema="schema"
                          :info="info"
        ...
        <Modal v-model="deleteModal" :closable="false" width="360">
            <div style="color:#ed4014;text-align:center">
                <p>Та уг мэдээллийг устгахдаа итгэлтэй байна уу.</p>
            </div>
            <div slot="footer" style="text-align: center">
                <Button type="default" size="small" @click="deleteModal = false">Үгүй</Button>
                <Button type="error" size="small" :loading="delLoading"
                        @click="remove(currentRow.rowId, currentRow.rowIndex)">Тийм
                </Button>
            </div>
        </Modal>
    </div>
...
import {isValid, compareObj} from "./utils/methods";
import {convertLink} from "./utils/formula";
import Print from "./Print";
import ExcelImport from "./ExcelImport";
import DataFilter from "./DataFilter";
import {getNumber, number} from "./utils/number.js";
...
export default {
    ...
    methods: {
        ...
        initGrid(customSchemaId) {
            ...
            axios
                .get(this.page_id ? `/lambda/puzzle/schema/grid/${this.customShemaId ? this.customShemaId : this.$props.schemaID}?page_id=${this.page_id}` : `/lambda/puzzle/schema/grid/${this.customShemaId ? this.customShemaId : this.$props.schemaID}`)
                .then(({data}) => {
                    let gridSchema = JSON.parse(data.data.schema);
                    ...
                    //Getting data
                    this.query.src = "/lambda/puzzle/grid/data/" + data.data.id;
                    if (this.saveFilter) {
                        this.restoreFilter();
                    }
                    this.fetchData();
        ...
        getColumnWidth(item, colId) {

            let savedWidth = window.localStorage.getItem("column_width_" + colId);
        ...
        columnResized(event) {
            if (event.finished) {
                window.localStorage.setItem("column_width_" + event.column.colId, event.column.actualWidth);
            }
        },
        restoreFilter() {
            if (!this.isClient) {
                    ...
                    if (data.length >= 1) {
                        ...
                        if (colIndex >= 0) {
                            if (this.schema[colIndex].gridType == "Number") {
                                bottom_data[column] = data[0][aggregation + "_" + column]
                            } else {
                                bottom_data[column] = number(data[0][aggregation + "_" + column]) + " " + columnAggregation.symbol;
                            }
                        }
                ...
                .then(({data}) => {
                    ...
                    if (data.status) {
                        this.$Message.success({
                            content: 'Амжилттай хадгалагдлаа'
                        });
                        ...
                        this.$Message.error({
                            content: 'msg' in data ? data.msg : 'Хадгалах үед алдаа гарлаа!',
                        });
                    }
                })
                .catch(err => {
                    console.log(err);
                    this.$Message.destroy();
                    this.$Message.error('Хадгалах үед алдаа гарлаа!');
                })
        },
        ...
        saveGridData() {
            if (this.changedRowsData.length == 0) {
                this.$Message.info({
                    content: 'Өөрчлөлт орсон мэдээлэл байхгүй байна.'
                });
                return;
            }
