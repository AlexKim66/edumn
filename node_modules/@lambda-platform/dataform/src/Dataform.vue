<template>
  <div :class="viewMode ? 'dataform view-mode' : 'dataform'">
    <Form :ref="meta.model +'-'+ schemaID" :model="model" :rules="rule" :label-position=meta.option.labelPosition
      ...
      <div class="dataform-footer" v-if="!viewMode">
        <Button type="info" :loading="asyncMode" @click="handleSubmit(meta.model +'-'+ schemaID)">
          <span v-if="!asyncMode">{{ $static_words ? $static_words.save : save_btn_text && save_btn_text != '' ? save_btn_text : 'Хадгалах' }}</span>
          <span v-else>{{`${$static_words ? $static_words.pleaseWaitLoading : 'Түр хүлээнэ үү...'}`}}}</span>
        </Button>
    ...
    >
      ...
      <div slot="footer">
        <Button type="primary" size="large" @click="showInfo = false">{{`${$static_words ? $static_words.close : 'Хаах'}`}}}</Button>
      </div>
    </Drawer>
  </div>
</template>
